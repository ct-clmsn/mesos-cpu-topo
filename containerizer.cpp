...
#ifdef __NUMATOPOLOGY_RESOURCE__
  #include "numatopology.hpp"

  // compiles numa domain information
  //
  if(!strings::contains(flags.resources.getOrElse(""), "numatopology")) {

    Option<NumaTopologyDescription> numainfodesc = NumaTopologyDescription::compile();
    if(numainfodesc) {
      resources += Resources::parse(
        "numatopology",
        stringify(numainfodesc.get()),
        flags.default_role).get();
    }
  }
